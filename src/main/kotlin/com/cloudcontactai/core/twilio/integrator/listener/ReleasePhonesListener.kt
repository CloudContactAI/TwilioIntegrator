...
package com.cloudcontactai.core.twilio.integrator.listener
...
@Component
class ReleasePhonesListener {
    @Autowired
    lateinit var twilioPhoneService: TwilioPhoneService

    @JmsListener(destination = "\${twilio.queue.releaseTwilioPhones}")
    fun receiveMessage(message: String) {
        try {
            val typeRef: TypeReference<List<String>> = object : TypeReference<List<String>>() {}
            val phones = ObjectMapper().readValue(message, typeRef)
            twilioPhoneService.delete(phones)
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }
